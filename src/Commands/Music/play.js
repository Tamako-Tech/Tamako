  ...
  run: async (client, message, args) => {
    ...
    try {
      ...
      if (_checkURL(query)) {
        ...
        if (!result?.tracks.length) { return await message.reply('I didn\'t find anything in the query you gave me') }
        ...
        if (dispatcher === 'Busy') { return await message.reply('I\'m already playing music in this guild!') }
        if (playlist) {
          for (const track of result.tracks) { await client.queue.handle(message.guild, message.member, message.channel, node, track) }
        }
      ...
      if (!search?.tracks.length) { return await message.reply('I didn\'t find anything in the query you gave me') }
