...
module.exports = {
    name: 'setchatbotch',
    aliases: [ ],
    description: 'Set up the chatbot channel',
    ownerOnly: false,
    cooldown: 0,
    ...
    run: async (client, message, [ channel = '' ]) => guilds.findById(message.guild.id, (err, doc) => {
        ...
        doc.channels.chatbot = channel.id;
        return doc.save()
            .then(() => {
                client.guildProfiles.get(message.guild.id).featuredChannels.chatbot = channel.id;
                return message.channel.send(`\\✔️ Successfully set the chatbot channel to ${channel}!`);
            })
            .catch((err) => {
                console.log(err);
