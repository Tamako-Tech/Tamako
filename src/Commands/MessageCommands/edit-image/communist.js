    ...
    run: async (client, message, [ image ], container) => {
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-image/communist`)
                .query({ image: image });
            ...
            return message.reply({ embeds: [embed], files: [{ attachment: body, name: 'communist.png' }] });
        } catch(err) {
