...
module.exports = {
    ...
    category: 'Edit-Image',
    usage: '[image]',
    run: async (client, message, [ level, image ], container) => {
        if (!level) return message.reply('Please provide a level of desaturation.');
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-image/desaturate`)
                .query({ level: level, image: image });

            if (Buffer.byteLength(body) > 8e+6) return message.reply('Resulting image was above 8 MB.');
        
            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://desaturate.png')  
            ...
            return message.reply({ embeds: [embed] ,files: [{ attachment: body, name: 'desaturate.png' }] });
        } catch(err) {
