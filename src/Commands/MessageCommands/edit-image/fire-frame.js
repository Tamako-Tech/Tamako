    ...
    run: async (client, message, [ image ], container) => {
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-image/fireframe`)
                .query({ image: image  });
            ...
            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://fire-frame.png')
            ...
            return message.reply({ embeds: [embed] ,files: [{ attachment: body, name: 'fire-frame.png' }] });
