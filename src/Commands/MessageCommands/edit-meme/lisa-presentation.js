...
module.exports = {
    ...
    run: async (client, message, [ ...text ], container) => {
        text = text.join(' ');
        if (!text) return message.reply('What should the text of the presentation be?');
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-meme/lisa-presentation`)
                .query({ text: text });
            
            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://lisa-presentation.png')
            ...
            return message.reply({ embeds: [embed], files: [{ attachment: body, name: 'lisa-presentation.png' }] });
        } catch(err) {
