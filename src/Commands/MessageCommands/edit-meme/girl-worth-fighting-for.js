    ...
    run: async (client, message, [ image ], container) => {
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-image/girl-worth-fighting-for`)
                .query({ image: image });

            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://girl-worth-fighting-for.png')
            ...
            return message.reply({ embeds: [embed], files: [{ attachment: body, name: 'girl-worth-fighting-for.png' }] });
        } catch(err) {
