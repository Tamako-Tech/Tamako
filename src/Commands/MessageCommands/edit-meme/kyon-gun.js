    ...
    run: async (client, message, [ image ], container) => {
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-image/kyonGun`)
                .query({ image: image });

            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://kyonGun.png')
            ...
            return message.reply({ embeds: [embed], files: [{ attachment: body, name: 'kyonGun.png' }] });
        } catch(err) {
