    ...
    run: async (client, message, [ image ], container) => {
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-image/ThisGuy`)
                .query({ image: image });

            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://ThisGuy.png')
            ...
            return message.reply({ embeds: [embed], files: [{ attachment: body, name: 'ThisGuy.png' }] });
        } catch(err) {
