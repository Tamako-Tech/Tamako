    ...
    run: async (client, message, [ image, ...name ], container) => {
        ...
        try {
            const { body } = await request
                .get(`${process.env.API_URL}/canvas/edit-meme/enslaved`)
                .query({ name: name, image: image });
            
            const embed = new container.Discord.MessageEmbed()
                .setColor('GREY')
                .setImage('attachment://enslaved.png')
            ...
            return message.reply({ embeds: [embed], files: [{ attachment: body, name: 'enslaved.png' }] });
        } catch(err) {
