...
const schedule = require('require-text')(join(__dirname, '..', '..', 'assets', 'graphql', 'Schedule.graphql'), require);
const watchlist = require(join(__dirname, '..', '..', 'Models', 'GuildWatchlist'));
const anilist = require(join(__dirname, '..', '..', 'Utils', 'anilist' ));
const constants =  require(join(__dirname, '..', '..', 'Utils', 'constants' ));
...
module.exports = class Anischedule{
    ...
    getAnnouncementEmbed(entry, date){
        ...
        return new MessageEmbed()
            ...
            .setAuthor({name: 'Tamako Anischedule'})
            .setTimestamp(date)
            .setDescription([
    ...
    async handleSchedules(nextDay, page){
        ...
        if (res.errors){
            return console.log(`ANISCHEDULE: FetchError\n${res.errors.map(err => err.message).join('\n')}`);
        }
        ...
        for (const e of res.data.Page.airingSchedules){
            ...
            console.log(`Tracking Announcement for Episode \x1b[36m${
                e.episode
            }\x1b[0m of \x1b[36m${
    ...
    async init(){
        if (!this.enabled){
            return;
        } else if (!this.client.dbConnected){
            console.log('Unable to start Anischedule. Reason: Database not found. Retrying...', 'db');
        } else {
            // Do nothing
        }
    ...
    async makeAnnouncement(entry, date){
        ...
        for (const g of list){
            ...
            await channel.send({embeds: [embed]})
                .then((msg) => {
                    console.log(`Announcing episode \x1b[36m${
                        entry.media.title.romaji || entry.media.title.userPreferred
                    }\x1b[0m to \x1b[36m${
                        channel.guild.name
                    }\x1b[0m @ \x1b[36m${
                    ...
                    console.log(`Announcement for \x1b[36m${
                        entry.media.title.romaji || entry.media.title.userPreferred
